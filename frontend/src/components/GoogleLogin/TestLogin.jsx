...
function TestLogin() {
    ...
    const login = useGoogleLogin({ // Functionality for the Google Login Button 
        ...
        onSuccess: async (response) => { // if the system operates successfully, retrieve credential information from Google OAuth Service
            ...
            try {
                console.log('Response from server:', response); // response: json
                // response contains: access_token, token_type, expires_in, scope, authuser, prompt

                const res = await axios.post('http://localhost:3000/auth/google', response ); // fetch result from server
                if (res.data.token) { // if the server successfully returns a response 
                    document.cookie = `jwt=${res.data.token}; path=/; HttpOnly`;
                    setMessage("Welcome, " + res.data.user.name); // changed text
                    setProfile(res.data.user); // change profile 
    ...
    const logOut = () => { // log out functionality of the button 
        ...
        document.cookie = 'jwt=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT'; // update the cookies and jwt to none
    };
